...
class adapter_pilih_pelanggan(
    private val originalList: List<modelpelanggan>

) : RecyclerView.Adapter<adapter_pilih_pelanggan.ViewHolder>() {

    private var filteredList: ArrayList<modelpelanggan> = ArrayList(originalList)
    lateinit var appContext: android.content.Context

    fun filterList(keyword: String) {
        filteredList = if (keyword.isBlank()) {
            ArrayList(originalList) // Tampilkan semua data
        } else {
            ArrayList(originalList.filter {
                it.tv_nama_pelanggan?.contains(keyword, ignoreCase = true) == true
            })
        }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.tvAlamat.text = "${item.tv_alamat_pelanggan}"
        holder.tvNoHP.text = "${item.tv_nohp_pelanggan}"
