...
class transaksi : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnProsesTransaksi.setOnClickListener {
            ...
            val idTransaksi = "TRX" + System.currentTimeMillis().toString()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == pilihLayanan) {
            if (resultCode == RESULT_OK && data != null) {
                ...
                tvTRANSAKSI_layanan_harga.text = "Harga : ${formatRupiah(harga ?: "0")}"
    ...
    private fun formatRupiah(hargaString: String): String {
        val localeID = Locale("in", "ID")
        val format = NumberFormat.getCurrencyInstance(localeID)
        val clean = hargaString.replace("Rp", "")
            .replace(".", "")
            .replace(",", ".")
            .trim()
        val harga = clean.toDoubleOrNull() ?: 0.0
